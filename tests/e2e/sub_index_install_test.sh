...
echo "$INSTALL_NS_OUT"
echo "$INSTALL_NS_OUT" | grep -q "installed" \
  || fail "install testd2x:d2testpkg failed"
...
echo "$INSTALL_BARE_OUT" | grep -q "installed" \
  || fail "install d2testpkg (bare name) failed - sub-repo packages should be installable by bare name when unambiguous"
